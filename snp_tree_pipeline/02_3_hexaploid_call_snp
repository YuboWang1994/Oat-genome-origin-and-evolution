...
bwa mem -R "@RG\tID:{sample}\tLB:{sample}\tPL:ILLUMINA\tSM:{sample}" -t 10 {path}/00_genome/ACD/ACD_index/ACD.fa {sample}.R1.fq.gz {sample}.R2.fq.gz -o {sample}.sam && touch Step1_done

samtools view -h {sample}.sam |grep -v 'XA:Z' |grep -v 'SA:Z'| awk '$3!~"chr[0-9]C"' |awk '$3!~"chr[0-9]D"'| awk '$3!~"Contig"'|samtools fastq -1 A.R1.fq -2 A.R2.fq -

samtools view -h {sample}.sam |grep -v 'XA:Z' |grep -v 'SA:Z'| awk '$3!~"chr[0-9]A"' |awk '$3!~"chr[0-9]D"'| awk '$3!~"Contig"'|samtools fastq -1 C.R1.fq -2 C.R2.fq -

samtools view -h {sample}.sam |grep -v 'XA:Z' |grep -v 'SA:Z' |awk '$3!~"chr[0-9]A"'| awk '$3!~"chr[0-9]C"'| awk '$3!~"Contig"'|samtools fastq -1 D.R1.fq -2 D.R2.fq -

seqkit seq -n -i A.R1.fq |sort -T tmp -u > A.R1.fq.list
seqkit seq -n -i A.R2.fq |sort -T tmp -u > A.R2.fq.list
...
awk '{if(NR==FNR){a[$1]}else if($1 in a)print $0}' A.R1.fq.list A.R2.fq.list > A.list
...
perl {path}/fishInWinter.pl -bf table -ff fq -gene A.list A.R1.fq | gzip > A.R1.fq.gz
perl {path}/fishInWinter.pl -bf table -ff fq -gene A.list A.R2.fq | gzip > A.R2.fq.gz
...
# A subgenome data alignment

mkdir -p {path}/{sample}/A_bam/A
cd {path}/{sample}/A_bam/A
...
mkdir -p {path}/{sample}/A_bam/C
cd {path}/{sample}/A_bam/C
...
mkdir -p {path}/{sample}/A_bam/D
cd {path}/{sample}/A_bam/D
